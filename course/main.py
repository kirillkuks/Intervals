...
from linear_regression import LinearRegression, LinearTwinRegression, Plotter, RemainderAnalyzer
    ...
    def create_interval_sample(self, min_max: bool = True) -> List[Interval]:
        ...
        for i in range(self.data_sample.factors_num()):
            ...
            if min_max:
                ...
                avg = (mi + ma) * 0.5
                diff = (ma - mi) * 0.5 * 0.25
                responses.append(Interval(np.quantile(sample, 0.375), np.quantile(sample, 0.625)))
            else:
                med = median(sample)
...
def main():
    ...
    responses_inner = data_builder.create_interval_sample(False)
    responses_outer = data_builder.create_interval_sample(True)

    responses_twin = [Twin(inner, outer) for inner, outer in zip(responses_inner, responses_outer)]

    twin_regression = LinearTwinRegression(data_sample.factors, responses_twin)
    twin_regression.build_point_regression()
    twin_regression.build_inform_set()

    plotter = Plotter(True)

    name = 'X1'
    plotter.plot_twin_sample(twin_regression.x, twin_regression.y, True, name)
    plotter.plot_twin_corridor(twin_regression, True, 5, 5, name)
    plotter.plot_twin_inform_set(twin_regression, name)
    twin_regression.print_point_regression_params()

    remainder_analyzer = RemainderAnalyzer()
    twin_remaindes = remainder_analyzer.build_twin_remainders(twin_regression.x, twin_regression.y, twin_regression)

    remainders_twin_regression = LinearTwinRegression(data_sample.factors, twin_remaindes)
    remainders_twin_regression.build_point_regression()
    remainders_twin_regression.build_inform_set()

    name = 'E1'
    plotter.plot_twin_sample(remainders_twin_regression.x, remainders_twin_regression.y, True, name)
    plotter.plot_twin_corridor(remainders_twin_regression, True, 5, 5, name)
    plotter.plot_twin_inform_set(remainders_twin_regression, name)
